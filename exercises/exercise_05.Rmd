...
full_join(time1_df, time2_df, by = 'id')
```
...
```{r}
inner_join(time1_df, time2_df, by = 'id')
```
...
```{r}
left_join(time1_df, time2_df, by = 'id')
```
...
```{r}
anti_join(time1_df, time2_df, by = 'id')
```
...
```{r}
gathered_df <- inner_join(time1_df, time2_df, by = 'id') %>%
